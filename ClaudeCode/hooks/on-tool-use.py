...
def main():
    ...
    important_tools = ['Read', 'Edit', 'Bash', 'Write']
    ...
    # Store if it seems important (file edits, significant bash output)
    if tool_name in ['Edit', 'Write'] or (tool_name == 'Bash' and len(str(tool_output)) > 500):
        content = '\n'.join(content_lines)
        try:
            entry = memory.store(
                content=content,
